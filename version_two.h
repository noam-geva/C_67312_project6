...
public:
    ...
    Iterator Insert(Iterator it, InputIterator first, InputIterator last) noexcept {
        size_t ret_index = it - this->begin();
        size_t count = 0;
        ...
        this->SizeUpCap();
        for (size_t ix = this->size_; ix > ret_index ; ix--){
            this->data_[ix] = this->data_[ix - count];
        }
        for (; first != last ; first++) {
            *it = *first; //dereference it = dereference first
            it++;
        }
        return Iterator(ret_index);
    ...
    */
    ...
    public:
        ...
        explicit Iterator(const T *it ) : value_ptr_(it) {}
    ...
    public:
        ...
        explicit ConstIterator(const T* ptr ) : value_ptr_(ptr) {}

        // Conversion ctor
        explicit ConstIterator(const Iterator &it) : value_ptr_(it.value_ptr_) {}
    ...
    public:
        ...
        explicit RIterator(const T *it ) : value_ptr_(it) {}
    ...
    public:
        ...
        explicit RConstIterator(const T* ptr ) : value_ptr_(ptr) {}

        // Conversion ctor
        explicit RConstIterator(const RIterator &it) : value_ptr_(it.value_ptr_) {}
    ...
    RConstIterator rbegin() const noexcept{
        return Iterator(&this->data_[0]);
    }
